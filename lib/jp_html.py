...
game.createScoreboard = function()
{
	...
	for(var i = 1; i <= game.team_cnt; i++)
	{
		...
		content += "<td><h3 id='team" + i +"'>0</h3><input class='add-points' onclick='game.addPoints(" + i +  ")' type='image' src='img/add.png' /> <input class='subtract-points' onclick='game.subtractPoints(" + i +  ")' type='image' src='img/subtract.png' /></td>";
	}
...
'''
	...
	################################################################################
	def get_topic_boxes(self):
		content = ''
		max_rows = -1
		for topic in self._topics:
			if max_rows < 0 or max_rows > len(topic.get_boxes()):
				max_rows = len(topic.get_boxes())
		#print '%d columns, %d rows' % (len(self._topics), max_rows)
		# Let double jeopardy points show up in row three or higher.
		dj_row = random.randint(2, max_rows-1)
		dj_col = random.randint(2, len(self._topics)-1)
...
'''
			...
			for topic in self._topics:
				col_index += 1
				box = topic.get_boxes()[row_index]
				question = box.get_question()

				if dj_row == row_index and dj_col == col_index:
					#print 'DJ (c%d, r%d)' % (col_index, row_index)
					question += '''<br><img src="img/double_jeopardy.jpg"><br>(Choose to get either %d points or %d points.)<br>''' % (box.get_points(), box.get_points()*2)

				content += '''
						<td id="tq%(row_index)d%(col_index)d" class="cell clean" onclick="prompt.show('q%(row_index)d%(col_index)d', %(points)d)">
							<h3>%(points)d</h3>
							<div class="hide">
								<div id="q%(row_index)d%(col_index)d">%(answer)s</div>
								<div id="aq%(row_index)d%(col_index)d">%(question)s</div>
							</div>
						</td>
''' % { "col_index":col_index,
        "answer":box.get_answer(),
        "question":question,
...
<div id="options">
	<h1>Welcome to Jeopardy Trivia!</h1>
...
'''
	...
	def get_output_lines(self):
		...
		content += self.get_topic_columns()
		content += self.get_part_two()
		content += self.get_topic_boxes()
		content += self.get_part_three()
